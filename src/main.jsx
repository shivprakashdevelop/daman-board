...
function App(){
  ...
  const page = {
    ...
    Stats: <Stats listings={boardListings} recentBids={activity} homepageViews={homepageViews} onlineCount={onlineCount} />,
...
function Stats({listings: boardListings, recentBids: boardActivity, homepageViews, onlineCount}){
  ...
  const listingViews = boardListings.reduce((sum, item) => sum + (item.listingViews || 0), 0);
  const totalViews = homepageViews + listingViews;
  return <main className="stats-page page-wrap"><div className="page-intro"><h1>What Daman is<br/><em>looking at today.</em></h1><p>These totals are calculated from approved listings and recorded analytics.</p><span className="updated"><span className="pulse-dot"/> Live now · {onlineCount} online</span></div><div className="stat-grid"><Stat icon={<AppIcon icon={ViewIcon}/>} value={formatReach(totalViews)} label="views so far" /><Stat icon={<AppIcon icon={ViewIcon}/>} value={formatReach(totalReach)} label="Daman Reach" /><Stat icon={<AppIcon icon={Location01Icon}/>} value={onlineCount} label="online now" /><Stat icon={<AppIcon icon={Location01Icon}/>} value={liveCount} label="live spots" /><Stat icon={<AppIcon icon={TradeUpIcon}/>} value={`₹${formatINR(standingBids)}`} label="standing bids" /><Stat icon={<AppIcon icon={ChartLineData01Icon}/>} value={boardActivity.length} label="recent bids" /></div><div className="chart-card"><div className="chart-heading"><div><h2>Views so far</h2><span>{formatReach(homepageViews)} homepage · {formatReach(listingViews)} listing</span></div><span className="chart-label">Live</span></div><p className="empty-state">Time-series analytics will appear here as live events are recorded.</p></div><div className="stats-columns"><div className="data-card"><div className="data-heading"><h2>Highest Daman Reach</h2><span>Top 5</span></div>{boardListings.length ? [...boardListings].sort((a,b)=>b.clicks-a.clicks).slice(0,5).map((listing,index) => <div className="ranked-row" key={listing.id || listing.name}><span>{index + 1}</span><span className="mini-avatar">{listing.icon}</span><strong>{listing.name}</strong><b>{formatReach(listing.clicks)} reach</b></div>) : <p className="empty-state">No approved listing analytics yet.</p>}</div><div className="data-card"><div className="data-heading"><h2>Recent bids</h2><span>Live</span></div>{boardActivity.length ? boardActivity.slice(0,5).map((bid, index) => <div className="recent-stat-row" key={`${bid.name}-${index}`}><span className="mini-avatar">{bid.icon}</span><div><strong>{bid.name}</strong><small>{bid.action} · {bid.time}</small></div><b>₹{formatINR(bid.amount)}</b></div>) : <p className="empty-state">No bid activity recorded yet.</p>}</div></div><div className="info-note"><AppIcon icon={ShieldCheckIcon} size={20}/><span>Daman Reach is based on recorded unique reach. Views and actions are tracked separately from live Supabase analytics.</span></div></main>
}
